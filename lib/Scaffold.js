...
var Scaffold = function (opts) {
  ...
  _.extend(scaffold, {
    ...
    start: function (steps, done) {
      ...
      if (steps.length < 1) {
        return self.error('You failed to pass in ScaffoldSteps to this installer, please add some.');
      }
    ...
    startMultiInstall: function (steps, installOptions, done) {
      ...
      _.each(installOptions, function (opts) {
        installers.push(function (cb) {
          ...
          var tempInstaller = _.extend(_.cloneDeep(originalInstaller), {
            defaults: _.defaultsDeep(opts, _.cloneDeep(originalInstaller.defaults))
          });
          if (self._debug) {
    ...
    preInstall: function (err, answers) {
      ...
      if (!self.answers) {
        return self.error("The resulting answers was disrupted. Please review your steps to ensure they're not" +
          " destroying" +
          " the answers object.");
      }
      else if (self.answers.aborted) {
        ascii.aborted(self.content.aborted, true);
        return self.done.call(self); // don't use finalize when aborting, end things here
      }
        ...
        self.install.call(self, self.answers, function (err, stream) {
          ...
          if (stream) {
            return stream.on('end', function () {
              self.postInstall.call(self, self.answers, function () {
